...
With the language to implement introduced, we proceed by presenting our approach.
...
Our goal was to design a modular architecture striking a good balance between clearly separating concerns and limiting redundancies. The resulting class structure is depicted in the \textsc{uml} diagram depicted in \cref{fig:uml}. We briefly go through each component.
\begin{description}
...
The main challenge of implementing a brainfuq simulator lies in the representation of the quantum tape.
...
For readers interested in a standalone implementation of the simulator not using the sorrounding architecture, we refer to \texttt{brainfuq\_interpreter/brainfuq\_interpreter\_old.py}.

\subsection{Translation to \qiskit{}}
\label{sec:approach-translations}

Implementing the translator to \qiskit{} is comparatively straightforward, but still provided a few challenges. The first one was ensuring that the qubits on the resulting qiskit circuit follow the same order as the ones on the quantum tape, which we wanted to ensure. With this an on-the-fly construction of the circuit is out of question. To accomodate for this, we store all operations to apply with their the target index, potential quantum control index, and potential classical control index in the list \texttt{operations}. In \texttt{return\_state()}, where we know all existing qubits, we finally construct the \qiskit{} circuit.
Another challenge was determining how the \texttt{?} operation should be handled. In our implementation, we write each measurement to a new classical bit of the \qiskit{} circuit, and then realize \texttt{?} using the \texttt{if\_test()} method of qiskit circuits applied to the last used classical bit.
