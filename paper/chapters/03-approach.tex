...
Implementing the translator to \qiskit{} is more trivial.

One challenge was determining how the \texttt{?} operation should be handled. In our implementation, we write each measurement to a new classical bit of the \qiskit{} circuit, and then realize \texttt{?} using the \texttt{if\_test()} method of qiskit circuits applied to the last used classical bit.

Another challenge was ensuring that the qubits on the resulting qiskit circuit follow the same order as the ones on the quantum tape, which we wanted to ensure. With this an on-the-fly construction of the circuit is out of question. To accomodate for this, we store all operations to apply with their the target index, potential quantum control index, and potential classical control index in the list \texttt{operations}. In \texttt{return\_state()}, where we know all existing qubits, we finally construct the \qiskit{} circuit.
