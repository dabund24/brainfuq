...
As the gates of the \bfck{} specification form a universal basis for quantum gates, arbitrary, and in particular entangled, quantum states can be computed.
...
For instance $N=5$ produces the \bfck{} program \texttt{\textasciitilde +++++[\#\}*-]}.
...
This string is then embedded into the full \bfck{} program as the body to a loop which is executed for the optimal number of Grover iterations which we calculate classically for the given oracle.
...
Although this oracle is relatively compact, it still produces \bfck{} programs with a significant count of overall instructions.
For $N=6$, for instance, the \bfck{} program of a single Grover operator is comprised of 21255 classical instructions (including pointer shifts), which results in an overall runtime of almost $30s$.
...
Given the \bfck{} instructions presented in \Cref{sec:background}, there are some edge case inputs for which the expected behavior of the system is unclear.
