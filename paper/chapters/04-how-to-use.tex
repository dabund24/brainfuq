...
This section contains instructions on how our code can be used.

\subsection{Environment Setup}
\label{sec:use-env-setup}

To ensure dependencies do not conflict with system-wide Python installations, we recommend executing this project within an isolated virtual environment. 
After initializing and activating the environment from the project's root directory, the package can be installed along with its required dependencies in editable mode by running:
...
By utilizing the \texttt{-e} flag, the source code inside the \texttt{src/} directory is linked directly to the active environment. This allows any absolute imports to resolve automatically from anywhere on the system.

\subsection{Using the Interpreter to create a GHZ State}
\label{sec:use-interpreter}

In the following, we will look at the script \texttt{examples/ghz.py} (see Listing~\ref{lst:ghz_example}) to see how to interpret a \hb{} program that creates a GHZ-State and how to translate it into a \qiskit{} circuit.
...
program = "~" # put q0 in equal superposition
program += "+" * (N - 1) # loop counter
program += "[#}*-]" # loop body: add a cnot and decrement loop counter
interpreter.interpret_program(program)
print(simulator)

interpreter.reset() # reset interpreter before interpreting the next program
...
To facilitate prototyping, we provide a command line interface (\textsc{cli}) for our implementation. Using the \texttt{brainfuq simulate} command, a \hb{} program can be simulated, with the final quantum state optionally printed to stdout using the \texttt{-v} option. Using \texttt{brainfuq to-qiskit}, the \qiskit{} circuit equivalent to a \hb{} program is printed. More information about the \textsc{cli} can be obtained by running \texttt{brainfuq -h}.