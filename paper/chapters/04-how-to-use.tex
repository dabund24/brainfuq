...
After initializing and activating the environment from the project's root directory, the package can be installed along with its dependencies in editable mode by running:
...
In the following, we will look at the script \texttt{examples/ghz.py} (see Listing~\ref{lst:ghz_example}) to see how to interpret a \bfq{} program that creates a GHZ-State and how to translate it into a \qiskit{} circuit.
...
To facilitate prototyping, we provide a command line interface (\textsc{cli}) for our implementation. Using the \texttt{brainfuq simulate} command, a \bfq{} program can be simulated, with the final quantum state optionally printed to stdout using the \texttt{-v} option. Using \texttt{brainfuq to-qiskit}, the \qiskit{} circuit equivalent to a \bfq{} program is printed. More information about the \textsc{cli} can be obtained by running \texttt{brainfuq -h}.